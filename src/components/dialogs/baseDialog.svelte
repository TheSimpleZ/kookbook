<script>
  ...
  function getTop(perh) {
    return position == 'top' ? 0 : position == 'bottom' ? 100 - perh : 50 - perh / 2
  }
  ...
  function setMargins() {
    ...
    let mleft = -rect.x
    if (absolute) {
      mtop = position == 'top' ? elm_node.clientHeight : position == 'bottom' ? -m_node.parentNode.clientHeight : 0
      mleft = align == 'left' ? elm_node.clientWidth : align == 'right' ? -m_node.parentNode.clientWidth : 0
      mtop =
        position == 'middle' && align == 'center' ? -(m_node.parentNode.clientHeight - elm_node.clientHeight) / 2 : mtop
    } else {
      if (position == 'top' && align == 'center') {
        mtop = rect.height
      }
