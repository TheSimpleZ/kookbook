<script>
  ...
  $: books = orderBy(unorderedRecipes, [recipeSorters[orderByProperty] || orderByProperty], sortOrder)
  $: selectedRecipes = books.filter((r) => selectedRecipeIds.has(r.id))
...
<div class="flex flex-col h-full">
  ...
  />
  <Collection path="books" query={(ref) => ref.where(`readers`, 'array-contains', user.uid)} let:data={readableRecipes}>
    <Collection
      path="books"
